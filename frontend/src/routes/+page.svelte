<script lang="ts">
	...
	onMount(async () => {
		// 1. 데스크탑 앱 환경인 경우 백엔드에서 세션 가져오기
		if (
			window.pywebview &&
			window.pywebview.api &&
			window.pywebview.api.get_session
		) {
			try {
				const session = await window.pywebview.api.get_session();
				if (session && session.tokens && session.host) {
					// 세션이 있으면 스토어에 설정하고 이동 (검증은 publish 페이지 onMount에서 수행됨)
					user.set(session.tokens.user || { email: "User" }); // 임시 유저 객체
					kitsuHost.set(session.host);
					goto("/publish");
					return;
				}
			} catch (e) {
		...
		// 2. 브라우저 환경인 경우 로컬스토리지 확인 (하위 호환성)
	...
	async function handleLogin() {
		...
		try {
			...
			if (response.ok) {
				...
				// Store 저장
				user.set(data.user);
				kitsuHost.set(data.host);

				// Desktop 앱이면 백엔드에도 저장
				if (
					window.pywebview &&
					window.pywebview.api &&
...
<div
	class="min-h-screen flex items-center justify-center bg-slate-950 relative overflow-hidden text-slate-100 selection:bg-blue-500 selection:text-white"
>
	<!-- Decorative Background -->
	<div
		class="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950"
	></div>
	<div
		class="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none"
	>
		<div
			class="absolute -top-[20%] -left-[10%] w-[50%] h-[50%] bg-blue-600/10 rounded-full blur-[100px] animate-pulse"
		></div>
		<div
			class="absolute -bottom-[20%] -right-[10%] w-[50%] h-[50%] bg-purple-600/10 rounded-full blur-[100px] animate-pulse animation-delay-2000"
		></div>
	</div>

	<div
		class="max-w-md w-full bg-slate-900/50 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-slate-800 relative z-10"
	>
		<div class="text-center mb-8">
			...
			<p class="text-slate-400 text-sm mt-2">
				Sign in to batch upload your shots
			</p>
		</div>
		...
		>
			<div class="space-y-1.5">
				<label
					class="block text-xs font-bold text-slate-500 uppercase tracking-wider pl-1"
					for="host">Kitsu Host URL</label
				>
				<div class="relative">
					<input
						id="host"
			...
			{#if error}
				<div
					class="p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start gap-2 animate-in fade-in slide-in-from-top-2"
				>
					<svg
					...
					<p class="text-red-400 text-xs font-medium pt-0.5">
						{error}
					</p>
				</div>
			{/if}
	...
	<button
		...
		class="fixed bottom-6 right-6 text-slate-500 hover:text-slate-300 transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-widest z-20 group"
	>
